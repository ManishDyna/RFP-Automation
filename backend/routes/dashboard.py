...
async def save_schedule(request: Request, payload: dict = Body(...), user: dict = Depends(require_permission("schedule_automation.manage"))):
    try:
        ...
        # The row is a record only. Nothing downstream reads it — the cron
        # feature was removed and the real download cadence lives in Windows
        # Task Scheduler on the production VM.

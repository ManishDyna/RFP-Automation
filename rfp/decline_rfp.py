...
async def get_wizstep_position(page: Page, all_steps: bool = False) -> Optional[Any]:
    try:
        ...
        if not steps_locator:
            print("[ERROR] No wizBox with wizard steps found")
            return None
        ...
        print(f"[WARN] Error finding wizStep position: {e}")
        return None
...
async def go_to_step(page: Page, step_index: int, max_attempts: int = 3) -> Optional[int]:
    for attempt in range(max_attempts):
        try:
            ...
            if not steps_locator:
                print("[ERROR] steps_locator is None")
            ...
            if count < step_index:
                print(f"[ERROR] Not enough steps found (found {count}, needed {step_index})")
                await asyncio.sleep(1)
                continue
            ...
            try:
                ...
                print(f"[WARN] Normal click failed, trying JS click: {e}")
            ...
            if new_position == step_index:
                print(f"[OK] Successfully navigated to Step {step_index}")
                return step_index
            else:
                print(f"[WARN] Target step {step_index}, but current position is {new_position}")

        except Exception as e:
            print(f"[WARN] go_to_step attempt {attempt+1} failed: {e}")
            await asyncio.sleep(1)

    print(f"[ERROR] Failed to navigate to Step {step_index} after {max_attempts} attempts")
    return None
...
async def safe_click(page, selector: str):
    try:
        ...
        print(f"[WARN] safe_click failed for {selector}: {e}")
        return False
...
async def flow_of_process_according_to_step(page, current_position, title='-'):
    ...
    if current_position != 1:
        ...
        for _ in range(3):
            ...
            if result == 1:
                ...
                break
        if current_position != 1:
            print("[ERROR] Could not reach Step 1")
            try:
                from core.log_events import log_event
    ...
    if not decline_button_clicked:
        print("[ERROR] Failed to click 'Decline to Respond' button")
        try:
            from core.log_events import log_event
    ...
    try:
        ...
        """)
        print("[OK] Filled decline reason textarea")
        try:
            from core.log_events import log_event
        ...
        print(f"[WARN] Failed to fill textarea: {e}")
        try:
            from core.log_events import log_event
    ...
    print(f"[OK] Successfully declined RFP: {title}")
    try:
        from core.log_events import log_event
...
async def decline_rfps(page, data, company_name: str, rfp_id=""):
    ...
    for row in data:
        ...
        if rfp_id and rfp_ids_match(rfp_id, title):
            ...
            print(f"[OK] Found matching RFP: '{title}'")

    print(f"Found {len(filtered_data)} matching RFP(s) out of {len(data)} total RFPs")

    if not filtered_data:
        print(f"[WARN] No RFP found matching ID '{rfp_id}'")
        return []
    ...
    try:
        ...
        print(f"[WARN] Could not check Dataverse for decline status: {db_err}")

    if not filtered_data:
        print(f"[WARN] All matching RFPs are already declined")
        return []
    ...
    if not filtered_data:
        print(f"[WARN] No eligible RFPs to decline after filtering")
        return []
    ...
    async def attempt_decline_rfps(row):
        ...
        if not link:
            print(f"[WARN] No link for {title}, skipping.")
            return False
        ...
        try:
            ...
            if await flow_of_process_according_to_step(new_page, current_position, title=title):
                print(f"[OK] RFP '{title}' declined successfully.")
                ...
                try:
                    ...
                    print(f"[WARN] Error updating participation status: {status_err}")
                ...
                try:
                    ...
                    print(f"[WARN] Error logging decline event: {log_err}")
                await new_page.wait_for_timeout(10000)
                await new_page.close()
                return True
            else:
                print(f"[WARN] Failed to decline RFP: {title}")
                try:
                    from core.log_events import log_event
                    log_event("RFP", "Decline", "Fail", "Decline flow failed", title)
                except Exception as log_err:
                    print(f"[WARN] Error logging decline failure: {log_err}")
                await new_page.close()
                return False
                
        except Exception as e:
            print(f"[ERROR] Error processing RFP '{title}': {e}")
            try:
                from core.log_events import log_event
